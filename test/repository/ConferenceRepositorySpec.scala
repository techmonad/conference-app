...
class ConferenceRepositorySpec extends PlaySpecification {
  ...
  "Conference repository" should {
    ...
    "get all conferences" in new WithApplication() {
      val conferences: List[Conference] = await(conferenceRepo.getAll())
      conferences.length === 2
      conferences.head.name === "Scala Days"
    }

    "get conference by id" in new WithApplication() {
      val result: Option[Conference] = await(conferenceRepo.getById(1))
      result.isDefined === true
      result.get.name === "Scala Days"
    }

    "create conference" in new WithApplication() {
      val conferenceId: Int = await(conferenceRepo.create(Conference("Spark Summit", "scaladays@gmail.com", "Scala Days Conference", "FEBRUARY 7th, 2017", "FEBRUARY 9th, 2017", "Boston")))
      conferenceId === 3
    }


    "update a conference" in new WithApplication() {
      val result: Int = await(conferenceRepo.update(Conference("Scala Days", "scaladays@gmail.com", "Scala Days Conference", "April 18th, 2017", "April 21st, 2017", "Chicago", Some(1))))
      result === 1
    }

    "delete a row" in new WithApplication() {
      val result: Int = await(conferenceRepo.delete(1))
      result === 1
    }
  }
